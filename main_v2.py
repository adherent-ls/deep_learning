...
def main():
    ...
    dataloader1 = MutilDataLoader(
        [build_dataset],
        batch_size,
        collate
    )

    dataloader2 = MutilDataLoader(
        [build_dataset],
        batch_size * 16,
        collate
    )
    ...
    optimizer1 = torch.optim.Adam(model.Extraction.parameters(), lr=lr, betas=(0.9, 0.99),
                                  weight_decay=0.00004)
    optimizer2 = torch.optim.Adam(model.Prediction.parameters(), lr=lr, betas=(0.9, 0.99),
                                  weight_decay=0.00004)

    base_lr = [group['lr'] for group in optimizer1.param_groups]
    scheduler = Warmup(base_lr=base_lr, warm=10000)
    optimizer_with_scheduler1 = OptimizerWithScheduler(optimizer=optimizer1, scheduler=scheduler)
    optimizer_with_scheduler2 = OptimizerWithScheduler(optimizer=optimizer2, scheduler=scheduler)
    ...
    for i in range(epoch):
        bar1 = tqdm(dataloader1)
        bar2 = tqdm(dataloader2)

        bar_iter1 = iter(bar1)
        bar_iter2 = iter(bar2)
        while True:
            if index % split_step < 0.1 * split_step:
                try:
                    images, labels = next(bar_iter2)
                ...
                with torch.no_grad():
                    contextual_feature = model.Extraction(images)
                preds = model.Prediction(contextual_feature.contiguous())

                loss = criterion(preds, [texts, lengths])
                optimizer_with_scheduler2.zero_grad()
                loss.backward()
                curr_lr = optimizer_with_scheduler2.step()
                bar = bar2
            else:
                try:
                    images, labels = next(bar_iter1)
                except StopIteration as ret:
                    break
                texts, lengths = labels
                images = images.to(device)
                ...
                loss = criterion(preds, [texts, lengths])
                optimizer_with_scheduler1.zero_grad()
                loss.backward()
                curr_lr = optimizer_with_scheduler1.step()
                bar = bar1
