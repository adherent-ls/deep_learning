...
class CTCLossProxy(BaseLoss):
    def __init__(self, zero_infinity=False, use_focal_loss=False):
        ...
        self.ctc = CTCLoss(blank=0, zero_infinity=zero_infinity, reduction='none')
    ...
    def forward(self, preds, labels):
        ...
        loss = self.ctc(preds, text, preds_size, length)
...
class TotalVariationCrossEntropyLossProxy(BaseLoss):
...
class TotalVariationMarinCrossEntropyLossProxy(BaseLoss):
