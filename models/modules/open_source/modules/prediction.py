...
class Attention(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_classes, max_length):
    ...
    def forward(self, batch_H, text):
        ...
        num_steps = self.max_length  # +1 for [s] at end of sentence.
        ...
        if self.training:
            for i in range(num_steps):
                ...
                char_onehots = self._char_to_onehot(text[:, i], device=batch_H.device, onehot_dim=self.num_classes)
            ...
            for i in range(num_steps):
                char_onehots = self._char_to_onehot(targets, device=batch_H.device, onehot_dim=self.num_classes)
                hidden, alpha = self.attention_cell(hidden, batch_H, char_onehots)
