...
def adv():
    ...
    model = Model().model
    model.load_state_dict(torch.load(os.path.join(root, 'latest-adam_norelu.pth')))
    ...
    for data in valid_dataloader:
        ...
        x2, pred = model(image)
        pred = pred - torch.logsumexp(pred, dim=1, keepdim=True).detach()
        pred = pred / torch.norm(model.fc.weight, dim=1)
        ...
        for i in range(100):
            ...
            pred1 = pred_t - torch.logsumexp(pred_t, dim=1, keepdim=True).detach()
            pred1 = pred1 / torch.norm(model.fc.weight, dim=1)
            loss = criterion(pred1, pre_d.max(dim=-1)[1])
            loss.backward()
